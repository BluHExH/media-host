  ...
  return (
    <div className="mh-mesh min-h-screen">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        <div className="mb-3 rounded-xl border border-amber-200/80 bg-amber-50/80 px-3 py-2 text-xs text-amber-900">
          Video &amp; audio hosting is <strong>under development</strong>. Upload images or HTML for now.
        </div>
        <div className="mh-dropzone-glass relative py-12 text-center" onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); openUploadModal(e.dataTransfer.files); }}>
          <input ref={ref} type="file" accept="image/*,.html,.htm" multiple className="absolute inset-0 cursor-pointer opacity-0" disabled={uploading || !!pending} onChange={(e) => openUploadModal(e.target.files)} />
          <p className="text-sm font-semibold" style={{ color: "#1A2B3C" }}>{uploading ? "Uploading…" : "Drop images or HTML — video/audio under development"}</p>
        </div>
      ...
      {pending && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="mh-glass-strong w-full max-w-md p-6">
            ...
            <p className="mt-1 text-xs" style={{ color: "#5a6f82" }}>Images &amp; HTML only · video/audio under development</p>
