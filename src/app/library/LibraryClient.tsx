...
export default function LibraryClient() {
  ...
  const del = async (urls: string[]) => {
    ...
    const hard = folderFilter === "trash";
    const msg = hard
      ? (urls.length === 1 ? "Permanently delete this file from trash?" : "Permanently delete " + urls.length + " files from trash?")
      : (urls.length === 1 ? "Move this file to trash?" : "Move " + urls.length + " files to trash?");
    if (!confirm(msg)) return;
    const res = await authFetch("/api/delete", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ urls, permanent: hard }),
    });
    ...
    if (hard) {
      ...
      setFiles((prev) => {
        const next = prev.map((f) => (urls.includes(f.url) ? { ...f, album: "trash" } : f));
        if (folderFilter !== "all" && folderFilter !== "trash") {
          return next.filter((f) => !urls.includes(f.url));
        }
        return next;
      });
    }
    setSelected((prev) => { const n = new Set(prev); urls.forEach((u) => n.delete(u)); return n; });
    setError(data.message || (hard ? "Deleted permanently" : "Moved to trash"));
    if (!albums.includes("trash")) setAlbums((a) => [...a, "trash"].sort());
  };
    ...
    <div className="mh-mesh min-h-screen">
      ...
      <main className="mh-page mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="mh-toolbar">
          <input value={search} onChange={(e) => setSearch(e.target.value)} className="mh-input max-w-xs" placeholder="Search by name…" />
          ...
          {selected.size > 0 && (
            <>
              <button type="button" className="mh-btn mh-btn-outline px-3 py-1 text-xs" onClick={() => del(Array.from(selected))}>
                {folderFilter === "trash" ? "Purge" : "Trash"} {selected.size}
              </button>
              ...
              return (
                <div key={file.url} className={"mh-media-card overflow-hidden " + (selected.has(file.url) ? "is-selected" : "")}>
                  <div className="relative">
                    ...
                    />
                    {exp && (
                      <span className="absolute right-2 top-2 z-10 rounded-full bg-amber-100/95 px-2 py-0.5 text-[10px] font-semibold text-amber-900">{exp}</span>
                    )}
                  ...
                  <div className="mh-card-body p-3.5">
                    ...
                    <div className="mh-card-actions mt-2.5 flex flex-wrap gap-1.5">
                      ...
                      <button type="button" className="cursor-pointer rounded-full border border-red-100 bg-white/60 px-2 py-1 text-xs text-red-500" onClick={() => del([file.url])}>
                        {folderFilter === "trash" ? "Purge" : "Trash"}
                      </button>
                    </div>
                  </div>
                </div>
